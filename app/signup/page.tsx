...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface relative overflow-hidden">
      <div className="absolute inset-0 z-0 bg-gradient-to-tr from-[#556B2F]/10 to-[#8C5A3B]/5 mix-blend-multiply" />
      ...
      >
        <div className="text-center mb-4">
          <h1 className="text-3xl font-black text-on-surface tracking-tight mb-2">{t('auth.signup')}</h1>
          <p className="text-on-surface-variant">Choose how you want to join Bazaar</p>
        </div>
        ...
        >
          ...
          {t('auth.signup_as_customer')}
        </Link>

        <p className="mt-8 text-center text-sm text-on-surface-variant">
          {t('auth.already_have_account')}{" "}
          <Link href="/login" className="text-[#556B2F] font-bold hover:underline">
            {t('auth.login')}
          </Link>
        </p>
      </motion.div>
